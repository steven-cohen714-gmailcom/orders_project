...
# --- Config ---
DB_PATH = Path("/home/steven_cohen714/orders_project/data/orders.db")
BACKUP_DIR = DB_PATH.parent
TIMESTAMP = datetime.now().strftime("%Y%m%d_%H%M%S")
BACKUP_PATH = BACKUP_DIR / f"orders_backup_before_reset_{TIMESTAMP}.db"

# Tables to retain (config/static only)
STATIC_TABLES = {
...
def clear_non_static_tables(conn, all_tables):
    ...
    # Disable foreign key enforcement for safety during deletion
