...
@router.delete("/delete_attachment/{order_id}/{filename}")
def delete_attachment(order_id: int, filename: str):
    try:
        with sqlite3.connect("data/orders.db") as conn:
            cursor = conn.cursor()
        ...
        log_event("new_orders_log.txt", {"error": str(e), "type": "sqlite_delete"})
        raise HTTPException(status_code=500, detail=f"Database error during delete: {str(e)}")
    except Exception as e:
        log_event("new_orders_log.txt", {"error": str(e), "type": "delete"})
