...
async def get_pending_orders(
    ...
    try:
        ...
        filters.append("UPPER(o.status) IN ('PENDING','AWAITING AUTHORISATION','AUTHORISED','PARTIALLY PAID','FULLY PAID')")
...
async def get_cod_orders(
    ...
    try:
        ...
        # Always COD on this screen
        filters.append("UPPER(o.payment_terms) = 'COD'")
        filters.append("UPPER(o.status) IN ('PENDING','AWAITING AUTHORISATION','AUTHORISED','PARTIALLY PAID')")
        ...
        if requester and requester.strip().lower() != "all":
            filters.append("UPPER(r.name) LIKE UPPER(?)")
            params.append(f"%{requester.strip()}%")
        if supplier and supplier.strip().lower() != "all":
            filters.append("UPPER(s.name) LIKE UPPER(?)")
            params.append(f"%{supplier.strip()}%")

        # Optional status filter from the UI (e.g. Pending / Authorised / Partially Paid).
        if status and status.strip().lower() != "all":
            filters.append("UPPER(o.status) = UPPER(?)")
            params.append(status.strip())

        # Hide things we never want here
        # - Deleted
        # - Fully paid (by status)  — keep this if your “mark fully paid” really sets the status
        # - OR hide when amount_paid >= total (covers full/over payment even if status wasn’t updated)
        filters.append("UPPER(o.status) <> 'DELETED'")
        # Choose ONE of the following lines, depending on your workflow:

        # A) If you DO set status to 'Fully Paid' when the user marks it fully paid:
        filters.append("UPPER(o.status) <> 'FULLY PAID'")

        # B) Or, if you prefer the robust numeric test instead (uncomment this and remove the line above):
        # filters.append("(o.amount_paid IS NULL OR o.amount_paid < o.total)")
        ...
        # Normalise date to YYYY-MM-DD for the UI
        for o in orders:
            try:
                if o["created_date"]:
                    for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%d"):
                        try:
                            o["created_date"] = datetime.strptime(o["created_date"], fmt).strftime("%Y-%m-%d")
                            break
                        except ValueError:
                            continue
                    else:
...
async def mark_cod_paid(order_id: int, payload: CodPaymentPayload, user: Dict = Depends(require_login)):
    ...
    try:
        ...
        # --- Eligibility guard for COD payment ---
        row = cursor.execute(
            "SELECT status, payment_terms FROM orders WHERE id = ?",
            (order_id,)
        ).fetchone()
        if not row:
        ...
        if terms_up != "COD" or status_up not in ALLOWED_STATUSES:
