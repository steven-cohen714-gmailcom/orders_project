...
# Ensure all these helper functions are 'async def' if they are awaited or
# decorated with @handle_db_errors (which returns an async wrapper)
@handle_db_errors(entity="draft_order", action="creating")
async def _insert_draft_order_into_db(order_data: dict, items: list, current_user_id: int) -> dict: # CHANGED to async def
...
async def _get_draft_order_and_items_from_db(draft_id: int) -> dict: # CHANGED to async def
    with get_db_connection() as conn:
...
async def _update_draft_order_in_db(draft_id: int, order_data: dict, items: list, current_user_id: int): # CHANGED to async def
    with get_db_connection() as conn:
        cursor = conn.cursor()
...
async def _delete_draft_order_from_db(draft_id: int): # CHANGED to async def
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("DELETE FROM draft_order_items WHERE draft_order_id = ?", (draft_id,))
...
async def _get_all_draft_orders_from_db(): # CHANGED to async def
    with get_db_connection() as conn:
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute("""
...
async def create_new_draft_order(draft_order: DraftOrderCreate, request: Request):
    ...
    result = await _insert_draft_order_into_db(order_data, items_data, current_user_id) # CHANGED to await
...
async def get_pending_draft_orders(request: Request):
    # This endpoint can be permissioned if needed, e.g., only current user's drafts or all drafts for admin
    # user = request.session.get("user") 
    # if not user: raise HTTPException(status_code=401, detail="User not authenticated")
    
    draft_orders = await _get_all_draft_orders_from_db() # This line was correct and already awaited.
...
async def get_draft_order(draft_id: int, request: Request):
    # Permission check might be needed here too
    # user = request.session.get("user")
    # if not user: raise HTTPException(status_code=401, detail="User not authenticated")

    draft_order = await _get_draft_order_and_items_from_db(draft_id) # CHANGED to await
    return draft_order
...
async def update_draft_order(draft_id: int, draft_order_update: DraftOrderUpdate, request: Request):
    ...
    await _update_draft_order_in_db(draft_id, order_data, items_data, current_user_id) # CHANGED to await
...
async def delete_draft_order(draft_id: int, request: Request):
    ...
    await _delete_draft_order_from_db(draft_id) # CHANGED to await
