...
from datetime import datetime           # used for parsing query dates
from backend.utils.time_utils import now_utc_iso  # used for writing timestamps
from backend.database import get_db_connection    # shared connection helper
import logging
        ...
        with get_db_connection() as conn:
            ...
            for req in requisitions:
                if req["requisition_date"]:
                    try:
                        # Normalize any stored timestamp to YYYY-MM-DD
                        parsed = datetime.fromisoformat(str(req["requisition_date"]))
                        req["requisition_date"] = parsed.strftime("%Y-%m-%d")
                    except Exception:
                        logging.warning(f"Could not parse date for requisition {req['id']}: {req['requisition_date']}")
