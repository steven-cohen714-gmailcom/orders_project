...
from backend.utils.permissions_utils import require_login, require_can_edit_draft_orders
...
# CREATE (mutating) → must have can_edit_draft_orders
@router.post("")
async def create_new_draft_order(
    draft_order: DraftOrderCreate,
    user: dict = Depends(require_login),
    _: bool = Depends(require_can_edit_draft_orders),
):
    ...
    order_data = {
        ...
        "total": total,
        "order_note": draft_order.order_note,
        "note_to_supplier": draft_order.note_to_supplier,
...
# LIST (read-only) → login only
@router.get("/pending")
async def get_pending_draft_orders(user: dict = Depends(require_login)):
...
# GET single (read-only) → login only
@router.get("/{draft_id}")
async def get_draft_order(draft_id: int, user: dict = Depends(require_login)):
    draft_order = await _get_draft_order_and_items_from_db(draft_id)
    return draft_order

# UPDATE (mutating) → must have can_edit_draft_orders
@router.put("/{draft_id}")
...
# DELETE (mutating) → must have can_edit_draft_orders
@router.delete("/{draft_id}")
async def delete_draft_order(
    draft_id: int,
    _: dict = Depends(require_login),
    __: bool = Depends(require_can_edit_draft_orders),
):
    await _delete_draft_order_from_db(draft_id)
    return {"message": "Draft order deleted successfully"}

# FINAL SUBMIT (mutating) → must have can_edit_draft_orders
@router.post("/submit_final/{draft_id}")
async def submit_final_draft(
    draft_id: int,
    user: dict = Depends(require_login),
    _: bool = Depends(require_can_edit_draft_orders),
):
    """
    Converts a draft into a real order in strict 'For Review' state.
    - DOES NOT compute/assign required_auth_band (kept NULL).
    - DOES NOT send any emails.
    - Leaves banding + notifications to the review endpoint.
    """
    current_user_id = user["id"]

    # 1) Load draft + items
    draft = await _get_draft_order_and_items_from_db(draft_id)
    if not draft:
        raise HTTPException(status_code=404, detail="Draft order not found")

    # 2) Create real order in 'For Review' with NULL band
    with get_db_connection() as conn:
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        ...
        cursor.execute("""
            INSERT INTO orders (
                ...
                order_note, note_to_supplier,
                supplier_id, requester_id, payment_terms,
                required_auth_band, last_modified_by_user_id
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            draft["order_number"],
            "For Review",
            ...
            draft["total"],
            draft["order_note"],
            draft["note_to_supplier"],
            draft["supplier_id"],
            draft["requester_id"],
            draft["payment_terms"] or "On account",
            None,
            current_user_id
        ))
        ...
        """, (
            new_order_id,
            f"Draft {draft['order_number']} finalised by user {current_user_id} -> Order {new_order_id} in 'For Review'",
            current_user_id
        ))
    ...
    log_success("draft_order", "finalised",
                f"Draft {draft['order_number']} -> Order {new_order_id} (For Review). No band/emails here.")
    return {"message": "Draft submitted for review", "order_id": new_order_id}

# NEW: Get items for a specific draft order (read-only) → login only
@router.get("/api/items/{draft_id}")
@handle_db_errors(entity="draft_order_items", action="fetching")
async def get_items_for_draft_order(draft_id: int, user: dict = Depends(require_login)):
    with get_db_connection() as conn:
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute("""
