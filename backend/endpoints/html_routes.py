# File: backend/endpoints/html_routes.py
from fastapi import APIRouter, Request, HTTPException, Depends
from fastapi.templating import Jinja2Templates
from fastapi.responses import HTMLResponse
from backend.database import get_db_connection
from backend.utils.permissions_utils import require_screen_permission
import logging
...
# --- Review Orders screen (permission-gated, keep) ---
@router.get("/orders/review_orders", response_class=HTMLResponse)
async def review_orders_page(
    request: Request,
    _: bool = Depends(require_screen_permission("review_orders")),
):
    try:
        return templates.TemplateResponse("review_orders.html", {"request": request})
    except Exception as e:
        logging.error(f"Error rendering review orders page: {str(e)}")
        raise
