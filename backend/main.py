...
from backend.endpoints import email as email_router
from backend.endpoints.authorisations_api import router as authorisations_router
from backend.endpoints import audit_summary
# REMOVED: from backend.endpoints.email_service import router as email_service_router 
...
# REMOVED: from backend.endpoints.order_receiving import router as order_receiving_router
...
# --- NEW IMPORT: Import the new review_orders router ---
from backend.endpoints import review_orders

from backend.endpoints import reports

# --- Evolution Export router ---
from backend.endpoints.evolution_export import router as evo_router
...
@static_router.get("/reports", response_class=HTMLResponse,
                   dependencies=[Depends(require_login), Depends(require_screen_permission("reports"))])
...
@static_router.get("/reports/view", response_class=HTMLResponse,
                   dependencies=[Depends(require_login), Depends(require_screen_permission("reports"))])
async def reports_view_page(request: Request, group_by: str = "item"):
    user_screen_permissions = request.session.get("screen_permissions", [])
    return templates.TemplateResponse(
        "report_view.html",
        {
...
@static_router.get("/exports/evolution", response_class=HTMLResponse,
                   dependencies=[Depends(require_login), Depends(require_screen_permission("evo_export"))])
async def exports_evolution_page(request: Request):
    user_screen_permissions = request.session.get("screen_permissions", [])
    return templates.TemplateResponse(
        "exports_evolution.html",
        {
            "request": request,
            "user_screen_permissions": user_screen_permissions
        }
...
async def edit_order_page(request: Request, order_id: int):
    ...
    try:
        ...
        # --- NEW STATIC ROUTE: For Review Orders page ---
@static_router.get("/orders/review_orders", response_class=HTMLResponse,
                   dependencies=[Depends(require_login), Depends(require_screen_permission("review_orders"))])
async def review_orders_page(request: Request):
    user_screen_permissions = request.session.get("screen_permissions", [])
    return templates.TemplateResponse(
        "review_orders.html",
        {
...
app.include_router(authorisations_router)  # routes already start with /orders/api
app.include_router(audit_summary.router, prefix="/orders")

# Include all specific lookup routers first...
app.include_router(users_lookups_router, prefix="/lookups")
...
# Now the draft_orders router (has general parameterized routes)
...
# REMOVED: app.include_router(order_receiving_router,   prefix="/orders")
app.include_router(order_notes_router)
app.include_router(mark_cod_paid_api_module.router, prefix="/orders")
app.include_router(review_orders.router,     prefix="/orders")  # ← keep this one
app.include_router(reports.router,           prefix="/orders")
...
# REMOVED: app.include_router(email_service_router)

# Evolution export API (prefix="/evo" inside the router)
