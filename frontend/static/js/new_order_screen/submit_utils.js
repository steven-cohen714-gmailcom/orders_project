...
// Relative Path: frontend/static/js/new_order_screen/submit_utils.js

export async function submitOrder({
  currentOrderNumber, // The order number of the order being submitted
  authThresholds,     // Authorization thresholds (unused in review-first flow)
  ...
  console.log("submitOrder triggered (submitting for review)");
  await logToServer("INFO", "submitOrder started (for review)", { orderNumber: currentOrderNumber, draftId: draftId });
  ...
  if (!requesterId || !supplierId) {
    await logToServer("ERROR", "Missing requester or supplier for order", { requesterId, supplierId });
  ...
  if (!requestDate) {
    ...
    await logToServer("WARNING", "Missing request date for order");
    return false; // Return false on validation failure
  }
  ...
  try {
    items = Array.from(document.querySelectorAll("#items-body tr")).map((row) => {
      ...
      if (!itemCode || !project || qtyOrdered <= 0 || price <= 0) {
        throw new Error("Each item needs item, project, quantity > 0 & price > 0.");
      }
  ...
  if (items.length === 0) {
    ...
    await logToServer("WARNING", "No items added to order");
    return false; // Return false on validation failure
  }
  ...
  // New review flow: every non-draft submit goes "For Review".
  // No auth-band calculation here; that happens after the reviewer clicks ✅.
  const status = "For Review";
  const authBandRequired = null;
  ...
  const orderData = {
    ...
    items,
    // Keep existing behavior for drafts exactly as is:
    ...(draftId != null && String(draftId).trim() !== "" ? { draft_id: parseInt(draftId, 10) } : {}),
  };
    ...
    if (draftId) {
      try {
        ...
        if (!deleteDraftRes.ok) {
          ...
          await logToServer("WARNING", `Failed to delete original draft after final submission`, {
            draftId: draftId,
          ...
          await logToServer("INFO", `Original draft deleted after final submission`, {
            draftId: draftId,
            finalOrderId: data.order_id
          });
        }
        ...
        await logToServer("ERROR", `Exception deleting original draft`, {
          draftId: draftId,
          finalOrderId: data.order_id,
          exception: deleteErr.message
        });
      }
    }

    await logToServer("INFO", "Order submitted successfully (For Review)", {
