...
let authThresholds = [0, 0, 0, 0, 0]; // Initialized with 5 elements for all bands
...
// Load order number and all authorization thresholds
async function loadOrderNumber() {
  try {
      ...
      authThresholds = [
          ...
          parseFloat(settingsData.auth_threshold_5 || 0) // Ensure auth_threshold_5 is included
      ];
      console.log("Loaded thresholds:", authThresholds); // Debug to verify all 5 are loaded
  ...
  if (submitBtn) {
    submitBtn.addEventListener('click', () => {
      ...
      debounce(() => submitOrder({
        currentOrderNumber,
        authThresholds: authThresholds, // Updated to use authThresholds array with 5 elements
        itemsList,
        updateGrandTotal,
