...
export async function expandLineItemsForAudit(orderId, iconElement) {
  console.log(`Expanding audit line items for order ID: ${orderId}`);
  const currentRow = iconElement.closest("tr");
  const existingDetailRow = document.getElementById(`audit-items-row-${orderId}`);
  ...
  try {
    ...
    const newRow = document.createElement("tr");
    newRow.id = `audit-items-row-${orderId}`;
    const cell = document.createElement("td");
    cell.colSpan = currentRow.children.length;
    cell.style.padding = "1rem";
    ...
    if (!data.items || data.items.length === 0) {
      ...
      header.style.backgroundColor = "#f0f0f0";
      header.style.fontWeight = "bold";
      ["Item Code", "Description", "Project", "Qty Ordered", "Qty Received", "Received Date", "Price", "Total"].forEach(text => {
        const th = document.createElement("td");
        th.textContent = text;
        header.appendChild(th);
      });
    ...
    console.error("❌ Could not load audit line items:", err);
    alert(`❌ Could not load audit line items: ${err.message}`);
  }
}
