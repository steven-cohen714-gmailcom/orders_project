...
import { loadRequesters, loadSuppliers, loadStatuses, normaliseStatus } from './components/shared_filters.js';
...
async function loadFiltersAndOrders() {
    try {
        ...
        // ↓↓↓ NEW: populate Status via the shared component (single source of truth)
        loadStatuses("filter-status", { context: "pending" });
...
async function loadOrders() {
    ...
    // ↓↓↓ NEW: normalise status before sending (handles aliases like "Waiting for Approval")
    ...
    try {
        ...
        if (data.orders && Array.isArray(data.orders) && data.orders.length > 0) {
            data.orders.forEach((order, index) => {
                ...
                const receiveIconHTML = (["Pending","Authorised","Partially Paid","Partially Received","Fully Paid"].includes(rawStatus))
            ...
            `;
              ...
              if (["Pending","Authorised","Partially Paid","Partially Received","Fully Paid"].includes(rawStatus)) {
